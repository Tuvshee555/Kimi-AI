...
export async function POST(req: NextRequest) {
  ...
  if (!file)
    return NextResponse.json({ reply: "No image uploaded" }, { status: 400 });

  // 1. Upload image to Kimi file endpoint
  ...
  const compRes = await fetch("https://api.moonshot.cn/v1/chat/completions", {
    ...
    body: JSON.stringify({
      model: "kimi-latest", // vision-capable model
      messages: [
        {
          role: "user",
          content: [
  ...
  if (!compRes.ok)
    return NextResponse.json({ reply: "Vision failed" }, { status: 500 });
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
